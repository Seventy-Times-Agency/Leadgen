...
WELCOME_TEXT = (
    "👋 Привет! Я помогу собрать базу потенциальных клиентов.\n\n"
    "Скажи мне <b>нишу</b> (например: <i>стоматология</i>, <i>кофейня</i>, "
    "<i>барбершоп</i>) и <b>регион</b> — я найду компании через Google Maps "
    "и пришлю список с контактами.\n\n"
    f"Нажми «{SEARCH_BTN}», чтобы начать."
)
...
async def confirm_search(
    ...
    await message.answer(
        "🚀 Поиск запущен.\n\n"
        f"Ищу «{html_escape(niche)}» в регионе «{html_escape(region)}». "
        "Это может занять до минуты — я напишу, как только будут результаты.",
        reply_markup=main_menu(),
    )
